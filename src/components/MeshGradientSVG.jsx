...
export function MeshGradientSVG() {
  ...
  useEffect(() => {
    ...
    if (rect) {
      ...
      const maxOffset = 8;
      setEyeOffset({
        x: Math.max(-maxOffset, Math.min(maxOffset, (mousePosition.x - centerX) * 0.08)),
        y: Math.max(-maxOffset, Math.min(maxOffset, (mousePosition.y - centerY) * 0.08)),
      });
    }
  }, [mousePosition]);
      ...
      <svg xmlns="http://www.w3.org/2000/svg" width="231" height="289" viewBox="0 0 231 289" className="w-full h-auto">
        ...
        <foreignObject width="231" height="289" clipPath="url(#shapeClip)">
          <div className="w-full h-full">
            <MeshGradient className="w-full h-full" colors={colors} speed={1} />
          </div>
        </foreignObject>
